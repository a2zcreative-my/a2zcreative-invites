 ...
 */
function getDatabase(request: Request): any {
 ...
 */
export async function validateSession(request: Request, token: string) {
    const db = getDatabase(request);
 ...
 * @param {string} token - Session token
 */
export async function destroySession(request: Request, token: string) {
    if (!token) return;
